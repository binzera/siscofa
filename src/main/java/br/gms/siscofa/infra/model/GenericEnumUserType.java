package br.gms.siscofa.infra.model;
...
public class GenericEnumUserType implements UserType, ParameterizedType {
	...
	public Object nullSafeGet(final ResultSet rs, final String[] names,
		...
		final Integer value = rs.getInt(names[0]) == 0 ? null : rs
				.getInt(names[0]);
		if (value == null) {
			return value;
		...
		throw new HibernateException(value + " não foi encontrado no enum "
				+ this.enumClass);
	}

	@Override
	public void nullSafeSet(final PreparedStatement st, final Object value,
			final int index, final SessionImplementor sessionImplementor)
			throws HibernateException, SQLException {
		...
		if (en == null) {
			...
			throw new HibernateException(
					"Não foi possível definir um valor para o enum "
							+ this.enumClass);
		}
	}
	...
	public void setParameterValues(final Properties parameters) {
		if (parameters != null) {
			...
			try {
				...
				throw new IllegalArgumentException("A classe '" + classeEnum
						+ "' não existe, corrija o mapeamento!");
			}
		}
